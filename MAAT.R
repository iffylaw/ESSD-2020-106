...
#		DDTs = ∑Ts, Ts > 0
#		DDFs = ∑Ts, Ts < 0
# MAAT = (DDTa−DDFa)/P, P=365
#
# Reference:
# Riseborough, D., Shiklomanov, N., Etzelmuller, B., Gruber, S., and Marchenko, S.: 
#   Recent advances in permafrost modelling, Permafrost Periglac., 19, 137–156, 2008.
#   http://doi.org/10.1002/ppp.615
#
...
################################################################################
MAAT <- function(Year, TempName="Temperature", data=QTP_ATM) {
  
  maat <- sapply(Year, function(Year) tryCatch({
    (Thawing_index(Year=Year, TempName=TempName, data=data) - 
             Freezing_index(Year=Year, TempName=TempName, data=data))/365
